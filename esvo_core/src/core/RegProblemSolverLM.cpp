...
namespace esvo_core
{
  namespace core
  {
                            ...
                            bVisualize_(true)
    {
      ...
      {
        numDiff_regProblemPtr_ = std::make_shared<Eigen::NumericalDiff<RegProblemLM>>(camSysPtr_, rpConfigPtr_, NUM_THREAD_);
      }
    ...
    bool RegProblemSolverLM::solve_numerical()
    {
      ...
      while (true)
      {
        if (iteration >= rpConfigPtr_->MAX_ITERATION_)
          break;
        numDiff_regProblemPtr_->setStochasticSampling((iteration % numDiff_regProblemPtr_->numBatches_) * rpConfigPtr_->BATCH_SIZE_, rpConfigPtr_->BATCH_SIZE_);
        Eigen::VectorXd x(6);
        x.fill(0.0);
